...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800">Editar Miembro del Equipo</h1>
            <p class="mb-0 text-muted">Actualiza la información de <strong>{{ $teamMember->name }}</strong></p>
        </div>
            ...
            <form action="{{ route('admin.team.update', $teamMember) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('short_description') is-invalid @enderror" id="short_description" name="short_description" rows="3" maxlength="150">{{ old('short_description', $teamMember->short_description) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $teamMember->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
                                @endforeach
                            </select>
                            @error('user_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="active" name="active" value="1" {{ old('active', $teamMember->active) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-4">
                        <div class="mb-4">
                            <label for="image" class="form-label">Foto del Miembro {{ !$teamMember->image ? '<span class="text-danger">*</span>' : '' }}</label>
                            <div class="card mb-2">
                                <div class="card-body text-center py-3">
                                    <div class="image-preview mb-3" id="imagePreview">
                                        @if($teamMember->image)
                                            <img src="{{ Storage::url($teamMember->image) }}" alt="{{ $teamMember->name }}" class="img-fluid rounded-circle" style="width: 150px; height: 150px; object-fit: cover;">
                                        @else
                                    ...
                                    <div class="mb-2">
                                        <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image" accept="image/*" {{ !$teamMember->image ? 'required' : '' }}>
                                    </div>
                                    <small class="text-muted">
                                        @if($teamMember->image)
                                            Deja en blanco para mantener la imagen actual
                                        @else
                                            Debes subir una imagen (obligatorio)
                                        @endif
                                    </small>
                                    @error('image')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="social-networks-container">
                                @if(is_array($teamMember->social_networks) && count($teamMember->social_networks) > 0)
                                    @foreach($teamMember->social_networks as $index => $network)
                                        <div class="social-network-row mb-2">
                                            <div class="input-group">
                                                <span class="input-group-text">
                                                    <i class="bi {{ $network['icon'] ?? 'bi-globe' }}"></i>
                                                </span>
                                                <select class="form-select social-network-name" name="social_networks[{{ $index }}][name]" onchange="updateSocialIcon(this)">
                                                    <option value="facebook" {{ $network['name'] == 'facebook' ? 'selected' : '' }}>Facebook</option>
                                                    <option value="x" {{ $network['name'] == 'x' ? 'selected' : '' }}>X (Twitter)</option>
                                                    <option value="instagram" {{ $network['name'] == 'instagram' ? 'selected' : '' }}>Instagram</option>
                                                    <option value="linkedin" {{ $network['name'] == 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                                                    <option value="youtube" {{ $network['name'] == 'youtube' ? 'selected' : '' }}>YouTube</option>
                                                    <option value="github" {{ $network['name'] == 'github' ? 'selected' : '' }}>GitHub</option>
                                                    <option value="otro" {{ !in_array($network['name'], ['facebook', 'x', 'instagram', 'linkedin', 'youtube', 'github']) ? 'selected' : '' }}>Otro</option>
                                                </select>
                                                <input type="url" class="form-control" placeholder="URL (opcional)" name="social_networks[{{ $index }}][url]" value="{{ $network['url'] }}">
                                                <button type="button" class="btn btn-outline-danger remove-social-network" {{ count($teamMember->social_networks) <= 1 ? 'disabled' : '' }}>
                                ...
                                @else
                                    <div class="social-network-row mb-2">
                                        <div class="input-group">
                                            ...
                                            <input type="url" class="form-control" placeholder="URL (opcional)" name="social_networks[0][url]" value="">
                ...
                <div class="border-top pt-3 mt-4 text-end">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-1"></i> Actualizar Miembro
                    </button>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let socialNetworkCount = document.querySelectorAll('.social-network-row').length;
